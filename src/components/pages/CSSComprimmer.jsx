...
import Code from "../common/pageElements/PageCode/Code";

import CompressionMethod from "../common/dropdowns/CompressionMethod";

import CompressBtn from "../common/pageElements/PageCode/CompressBtn";
import SmartButton from "../common/pageElements/PageCode/CompressInteligentBtn";
...
const CSSPage = () => {
  ...
  const [codeString, setCodeString] = useState(
    `/* Estilos básicos para um container */
  .container {
    max-width: 1200px;
    margin: 0 auto;
    padding: 20px;
  ...
  const selectHandleUpload = (event) => {
    ...
    const validFiles = files.filter((file) => file.name.endsWith(".css"));
  ...
  return (
    <section className="flex justify-between pt-9 h-full max-xl:flex-col">
      ...
      <aside className="xl:fixed xl:z-[60] xl:h-full xl:top-0 xl:py-[5rem] max-xl:py-[3rem] bg-gray-900 max-xl:w-full max-xl:border-b xl:border-r border-slate-700 text-white p-4">
        ...
        <div className="my-4">
          <button
            ...
            className={`bg-blue-600 hover:bg-blue-700 text-white py-2 px-4 text-xs font-bold rounded-md w-full ${
              isFixed
                ? "fixed top-[6rem] max-md:top-20 rounded-none px-10 left-0 backdrop-blur-md bg-blue-700/50 right-0 z-0"
                : ""
            }`}
          >
            Importar Arquivo(s)
        ...
        >
          <i className="bi bi-filetype-css text-slate-600 text-4xl"></i>
          <h3 className="text-sm font-bold text-slate-700">
            Arraste seu(s) arquivo(s) até aqui
          </h3>
        </div>

        <div className="grid grid-cols-1 h-[300px] p-4 rounded-md border border-slate-800 my-4 gap-3">
          ...
          <div className="overflow-y-auto">
            {uploadedFiles.length > 0 ? (
              uploadedFiles.map((file, index) => (
                ...
                >
                  <div className="flex items-center gap-3">
                    <i className="bi bi-filetype-css text-xl text-slate-400"></i>
              ...
              <div className="flex flex-col gap-3 justify-center items-center w-full ">
                <i className="bi bi-filetype-css text-slate-600 text-5xl "></i>
      ...
      <div className="xl:pl-[29rem] xl:pr-4 flex-1 bg-gray-900 text-white p-4 overflow-x-hidden">
        <div className="flex gap-2 mb-3 pt-4">
          <CompressBtn
            ...
            typeCode={"css"}
          />
          <SmartButton codeString={codeString} typeCode={"css"} />
        </div>
        <div>
          <Code
            setHeight={"75vh"}
            ...
            codeEditable={true}
            setCodeString={setCodeString} // <-- Passa a função setCodeString
            codeString={codeString}
          />
        </div>
      </div>
